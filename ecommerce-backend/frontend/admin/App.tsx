...
export function App() {
  ...
  if (authState === "checking") {
    return <CenteredState title="正在校验后台登录态" text="跨境电商公司管理后台正在确认当前账号权限。" />;
  }
    ...
    return (
      <LoginPage
        onSuccess={(nextAccount) => {
          ...
          setAuthState("ready");
          setNotice({ type: "success", text: "已进入跨境电商公司管理后台。" });
          normalizeAdminPath(page);
        }}
  ...
  if (authState === "forbidden") {
    return (
      <CenteredState
        title="当前账号无权进入管理后台"
        text="请使用跨境电商公司管理员账号登录。普通用户可以回到跨境电商平台商城。"
        actionLabel="重新登录后台"
        onAction={() => {
          setAuthState("login");
          window.history.replaceState(null, "", "/admin/login");
