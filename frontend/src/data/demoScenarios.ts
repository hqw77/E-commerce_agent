...
const DEFAULT_SCENARIO_SET: DemoScenarioSet = {
  userId: "default",
  title: "综合演示",
  description: "默认问题覆盖综合后端，便于直接观察大模型路由、最终话术、Runtime Context、RAG、Tool、HITL 和降级。",
...
const DEMO_SCENARIO_SETS: Record<string, DemoScenarioSet> = {
  U1001: {
    userId: "U1001",
    title: "主路径演示",
    description: "张三拥有演示订单，用来验证大模型路由、订单物流、未发货退款、RAG、低置信和降级。",
    defaultQuestion: "电子发票通常多久能准备好？",
    scenarios: [
      {
        question: "电子发票通常多久能准备好？",
  ...
  U1002: {
    userId: "U1002",
    title: "FAQ、RAG 与身份边界",
    description: "李四没有绑定演示订单，默认用于观察身份上下文、FAQ、RAG 和安全边界。",
    defaultQuestion: "电子发票通常多久能准备好？",
    scenarios: [
      {
        question: "我是谁？",
...
const EARLY_LESSON_SCENARIOS: Record<number, DemoScenarioSet> = {
  2: {
    ...
    scenarios: [
      ...
      { question: "请用一句话介绍跨境电商客服", capability: "基础响应结构" },
    ],
  },
  3: {
    userId: "lesson-03",
...
const LATER_LESSON_SCENARIOS: Record<number, DemoScenarioSet> = {
  ...
  46: {
    ...
    scenarios: [
      ...
      { question: "如需运行证据，请回看综合后端", capability: "材料课边界" },
    ],
  },
};
