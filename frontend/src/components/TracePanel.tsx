      ...
      {toggles.reasoning ? (
        <div className="trace-section">
          ...
          <div className={`trace-card reasoning-content${response?.reasoning_content ? "" : " muted"}`}>
            ...
            <small>环境使用测试数据，可以把它作为 CoT/Reasoning 教学素材；真实客服终端通常改为展示脱敏摘要和 trace，避免暴露内部规则、未验证假设、工具细节或用户敏感信息。</small>
          </div>
        </div>
      ) : null}
  ...
  return (
    <div className="trace-section">
      ...
      <div className="trace-card">
        ...
        />
        <small>这里是项目的单次请求估算，不是模型服务商账单、预算审批或 FinOps 平台。</small>
      </div>
      ) : null}
    </div>
  );
